...
void guiInitInside(lv_obj_t *parent, uint16_t posY, uint16_t height, const char *name1, const char *name2){
    ...
    lv_obj_t * pressureBedroomIcon = lv_img_create(sensorsLine1BCon, NULL);
    lv_img_set_src(pressureBedroomIcon, &pressure36);
    lv_obj_align(pressureBedroomIcon, NULL, LV_ALIGN_IN_TOP_RIGHT, 0, 0);

    guiInsideObject.presureBedroomLabel = lv_label_create(sensorsLine1BCon, NULL);
    ...
    lv_obj_t * humidityRoomIcon = lv_img_create(sensorsLine2ACon, NULL);
    lv_img_set_src(humidityRoomIcon, &humidity36);
    lv_obj_align(humidityRoomIcon, NULL, LV_ALIGN_IN_LEFT_MID, 0, 0);

    guiInsideObject.humidityRoomLabel = lv_label_create(sensorsLine2ACon, NULL);
    ...
    lv_obj_align(sensorsLine2BCon, NULL, LV_ALIGN_IN_TOP_RIGHT, 0, 0);

    lv_obj_t * pressureRoomIcon = lv_img_create(sensorsLine2BCon, NULL);
    lv_img_set_src(pressureRoomIcon, &pressure36);
    lv_obj_align(pressureRoomIcon, NULL, LV_ALIGN_IN_TOP_RIGHT, 0, 0);

    guiInsideObject.presureRoomLabel = lv_label_create(sensorsLine2BCon, NULL);
...
void guiFillInsideAll(guiInsideValues_t* values){
	...
	chsnprintf(bufferTempBedroomLabel, sizeof(bufferTempBedroomLabel), "%.2f", (float)((float)values->tempBedroom / 100.0f) - 100);
	lv_label_set_text(guiInsideObject.tempBedroomLabel, bufferTempBedroomLabel);

	char bufferTempRoomLabel[10];
	chsnprintf(bufferTempRoomLabel, sizeof(bufferTempRoomLabel), "%.2f", (float)((float)values->tempRoom / 100.0f) - 100);
